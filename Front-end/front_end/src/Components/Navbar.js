...
const Navbar = () => {
  ...
  return (
    <div className="shadow-md w-full sticky left-0  md:w-auto bg-[#2C3A61]">
      <div className="md:flex items-center justify-between py-4 md:px-4 px-7">
        <div className="font-bold text-2xl cursor-pointer flex items-center justify-start font-[poppins] text-gray-800">
          <div className="text-3xl text-white mb-3 pt-2">
            <AiOutlineHome size={30} />
          </div>
          <div className="pt-2 relative left-3 bottom-1 text-white">GOHOME</div>
        </div>
        <div
        ...
        <ul
          className={`md:flex md:items-center md:pb-0 pb-12 absolute md:static bg-[#2C3A61] md:bg-transparent md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 pl-9 transition-all duration-500 ease-in ${
        ...
        >
          {Links.map((link) => (
            <li key={link.id} className=" md:ml-8 text-xl md:my-0 my-7">
              <a
                href={link.link}
                className="text-white hover:border-b-4 duration-500 "
              >
                {link.name}
              </a>
            </li>
