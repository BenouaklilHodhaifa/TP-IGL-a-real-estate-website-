...
const Ai = ({annonce}) => {
  ...
  return (
    <div className="bg-[#F5FBFF] shadow-1  rounded-lg h-full w-[350px] cursor-pointer  hover:shadow-2xl  p-1" onClick={()=>setShowModal(!showModal)}>
      <img
      ...
      {showModal ? (
        <>
          <div
            className="flex flex-col h-[1000px] inset-0 z-50 bg-[#F5FBFF] scroll-auto absolute" onClick={(e)=>{e.stopPropagation()}}
          >
            <div className="flex justify-end w-full p-5">
              <TfiClose size={30} className="text-[#4A65A1]" onClick={()=>{setShowModal(!showModal)}}/>
            </div>
            ...
            <div className="flex flex-row justify-between">
              <div className=" flex-1">
                <h6 className="text-xl font-extrabold text-[#4A65A1] my-2 mx-5">
                {titre}
                </h6>
                <p className="text-lg  my-2 mx-5">
                  {description}
                </p>
            ...
            <div className="flex justify-center flex-row mt-10">
              <form className="flex flex-row " onSubmit={(e)=>{/*ENVOYER MESSAGE ICI*/}}>
                <h6 className="text-xl font-extrabold text-[#4A65A1] my-3">
                  faire une offre : 
                </h6>
                <input onChange={()=>{setMessageChanged(true)}} class="resize-y shadow appearance-none border rounded py-4 px-3 text-gray-700 mb-3  focus:outline-none focus:shadow-outline mx-5" type="text" placeholder=""/>
